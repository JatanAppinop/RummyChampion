...
class ModalTransitionManager
{
    ...
    public ModalTransitionManager(List<AnimatedView> animatedViews, int defaultView = 0)
    {
    ...
    public ModalTransitionManager MoveToView(string viewName)
    {
    ...
    private void MoveOutCurrentViewToLeft()
    {
        ...
        _animatedViews[viewIndex].rectTransform.DOAnchorPos(new Vector2(-Screen.width, 0), 0.5f)
    ...
    private void MoveInViewfromLeft(int index)
    {
        ...
        rectTransform.DOAnchorPos(Vector2.zero, 0.5f).SetEase(Ease.InSine).OnComplete(() =>
        {
            _onComplete?.Invoke();
            _onComplete = null;
        });
