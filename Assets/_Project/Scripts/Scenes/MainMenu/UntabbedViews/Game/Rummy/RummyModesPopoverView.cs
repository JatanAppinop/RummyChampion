...
public class RummyModesPopoverView : PopoverView
{
    ...
    {
        OnlinePlayersLabel.SetText(OnlinePlayersContext.Instance.GetOnlinePlayersCountForRummy().ToString());
    }
    public void OnButtonClicked(string mode)
    {
        PopoverViewController.Instance.Show(PopoverViewController.Instance.rummyContestPopover,
        new KeyValuePair<string, object>(Appinop.Constants.KModeSelect, mode),
        new KeyValuePair<string, object>(Appinop.Constants.KModeSelect, "rummy")
        );
    }
