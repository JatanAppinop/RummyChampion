...
public class LudoContestPopoverView : PopoverView
{
    ...
    [Header("Color Choice")]
    [SerializeField] List<ColorSelectionButton> colorSelectionButtons;
    [SerializeField] ColorSelectionButton colorSelection;
    ...
    public override void Show(params KeyValuePair<string, object>[] args)
    {
        if (!isLoaded)
        {
            ...
            colorSelectionButtons.ForEach((b) =>
            {
                b.OnSelect.AddListener(OnColorChoiceButtonSelected);
            });
        }
        ...
        Debug.Log("Game Mode : " + args.GetValueByKey<string>(Appinop.Constants.KModeSelect));
        ...
        colorSelection = colorSelectionButtons.First();
        colorSelection.Select(true);
    ...
    private void UpdateList()
    {
        ...
        filteredData = DataContext.Instance.contestsData.FindAll(t => t.gameMode == gameType && t.isActive);
        List<TableData> TwoPlayerData = filteredData.FindAll(t => t.gameType == "2 Player").OrderBy(t => t.totalBet).ToList();
        List<TableData> FourPlayerData = filteredData.FindAll(t => t.gameType == "4 Player").OrderBy(t => t.totalBet).ToList();
        Debug.Log("Total Contest Data : " + filteredData.Count());

        UpdateTwoPlayerCells(TwoPlayerData);

        UpdateFourPlayerCells(FourPlayerData);
    ...
    public void UpdateFourPlayerCells(List<TableData> FourPlayerData)
    {
    ...
    public void ContestSelected(LudoContestButton selectedButton)
    {
        ...
        PlayerPrefs.SetString(Appinop.Constants.KPlayerColorSelect, colorSelection.color.ToString());
        Debug.Log("Selected Color : " + colorSelection.color.ToString());
    ...
    private void OnColorChoiceButtonSelected(MultiToggleButton button)
    {
        if (colorSelection == button)
        {
            return;
        }

        colorSelection = button as ColorSelectionButton;
