...
public class AadharKYCPopoverView : PopoverView
{
    [SerializeField] private bool isLoaded = false;

    [Space]
    [SerializeField] AdvancedInputField aadharInput;
    ...
    {
        if (string.IsNullOrEmpty(aadharInput.text))
        {
            aadharInput.ShowError("Please Enter Aadhar Number");
            return;
        }
        if (aadharInput.text.Length != 12)
        {
            aadharInput.ShowError("AADHAR Number should be 12 Digit");
            return;
        }

        if (imagePath == null || imagePath.Length < 1)
        {
            UnityNativeToastsHelper.ShowShortText("Please Select Your AADHAR Photo.");
            Debug.LogError("Please Select Your AADHAR Photo.");
            return;
        }
        ...
        data.Add("documentType", "Aadhaar");
        data.Add("documentNumber", aadharInput.text);
    ...
    public async void onSubmitButtonPressed_Auto()
    {
        if (string.IsNullOrEmpty(aadharInput.text))
        {
            aadharInput.ShowError("Please Enter Aadhar Number");
        ...
        string requestBodyJson = "{\"aadharNumber\":\"" + aadharInput.text + "\"}";
        //Call the PostForm function
        var responce = await APIServices.Instance.PostAsync<SimpleBaseModel>(APIEndpoints.aadharOtp, requestBodyJson, true);
