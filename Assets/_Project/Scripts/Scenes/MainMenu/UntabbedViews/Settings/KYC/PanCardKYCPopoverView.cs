...
public class PanCardKYCPopoverView : PopoverView
{
    [SerializeField] private bool isLoaded = false;

    [Space]
    [SerializeField] AdvancedInputField pancardInput;
    ...
    {
        if (string.IsNullOrEmpty(pancardInput.text))
        {
            pancardInput.ShowError("Please Enter Pancard Number");
            return;
        }
        if (!validatePanNumber(pancardInput.text))
        {
            pancardInput.ShowError("Enter a Valid Pancard Number");
            return;
        }

        if (imagePath == null || imagePath.Length < 1)
        {
            UnityNativeToastsHelper.ShowShortText("Please Select Your Pancard Photo.");
            Debug.LogError("Please Select Your Pancard Photo.");
            return;
        }
        ...
        data.Add("documentType", "Pancard");
        data.Add("documentNumber", pancardInput.text);
    ...
    private bool validatePanNumber(string str)
    {

        string strRegex = @"^[a-zA-Z]{3}[p|P|c|C|h|H|f|F|a|A|t|T|b|B|l|L|j|J|g|G][A-Za-z][\d]{4}[A-Za-z]$";
